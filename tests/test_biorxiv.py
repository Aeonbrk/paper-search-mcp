...
class TestBioRxivSearcherOffline(OfflineTestCase):
    def test_search_offline_matches_free_text_without_category_parameter(self):
        searcher = BioRxivSearcher()
        response = mock.Mock()
        response.raise_for_status = mock.Mock()
        response.json.return_value = read_fixture_json("biorxiv", "search_response.json")
        requested_urls = []
        ...
        with mock.patch(
            "paper_search_mcp.academic_platforms.biorxiv.request_with_retries",
            side_effect=fake_request,
        ) as mock_request:
            papers = searcher.search("single cell atlas", max_results=3)

        self.assertEqual(mock_request.call_count, 1)
        self.assertEqual(len(papers), 1)
        self.assertEqual(
            papers[0].title,
            "Single-cell atlas reveals adaptive immune signaling in lung repair",
        )
        self.assertEqual(papers[0].categories, ["cell biology"])
    ...
    def test_search_offline_returns_empty_for_nonsense_query(self):
        ...
        response = mock.Mock()
        response.raise_for_status = mock.Mock()
        response.json.return_value = read_fixture_json("biorxiv", "search_empty.json")

        with mock.patch(
            "paper_search_mcp.academic_platforms.biorxiv.request_with_retries",
            return_value=response,
        ) as mock_request:
            papers = searcher.search("quantum hedgehog entropy", max_results=3)
